...
@csrf_exempt
def issue_session_token(request) -> JsonResponse:
...
@api_view(["GET"])
@csrf_exempt
def validate_session_token(request) -> JsonResponse:
    # Get Token
...
@csrf_exempt
def refresh_session_token(request) -> JsonResponse:
...
def get_user_from_access_token(access_token_str: str) -> JsonResponse or None:
    try:
...
def getUserData(request, token: str) -> JsonResponse:
    try:
...
@csrf_exempt
def loginUser(request) -> JsonResponse:
    if request.method == "POST":
        data = json.loads(request.body)
        username = data.get("username")
        password = data.get("password")
...
@csrf_exempt
def logoutUser(request) -> JsonResponse:
    if request.method == "POST":
        logout(request)
...
def add_to_cart(request) -> JsonResponse:
    if request.method == "POST":
        ...
        guest_cart, created = models.GuestCart.objects.get_or_create(
...
@csrf_exempt
def update_cart(request) -> JsonResponse:
    return JsonResponse({"message": "Logout successful. "}, status=200)


@csrf_exempt
def merge_cart(request) -> JsonResponse:
    return JsonResponse({"message": "Logout successful. "}, status=200)


@api_view(["GET"])
@csrf_exempt
def get_cart(request) -> JsonResponse:
    print("getting cart...")
    ...
    raw_cart_items = models.GuestCartItems.objects.filter(
